...
void depth_to_cloud(const cv::Mat& rgb_img, const cv::Mat& depth_img,
                    const Eigen::Matrix4f transToWorld,
                    CloudXYZRGB::Ptr& point_cloud, bool downsample = false) {
  pcl::PointXYZRGB pt;
...
visualization_msgs::MarkerArray cuboids_to_marker(object_landmark* obj_landmark,
  ...
  visualization_msgs::Marker marker;
  if (obj_landmark == nullptr) return plane_markers;
...
void publish_all_poses(std::vector<tracking_frame*> all_frames,
  ...
  std::cout << __FUNCTION__ << " total_frame_number is " << total_frame_number
            << endl;
  ...
  if (save_results_to_txt)  // record cam pose and object pose
  {
	...
	cout << __FUNCTION__ << " Finished to save pose to " << objresultsPath << endl;
  }
  ...
  while (n.ok()) {
    ...
    if (frame_number < total_frame_number) {
      ...
      std::string raw_rgb_img_name = base_folder + "raw_imgs/" +
                                     ...
                                     "_rgb_raw.jpg";
      std::cout << __FUNCTION__ << " Reading images again: " << raw_rgb_img_name
                << endl;
 ...
 * 當online_detect_mode　＝　ｔｒｕｅ時，只有最後一個變量truth_frame_poses是會被使用的；且只使用truth_frame_poses中的第一幀乳香的位姿
 * ｃａｍｐｏｓｅ以四元數+平移的方式存儲：Ｔｗｃ： x y z qx qy qz qw
 */
...
void incremental_build_graph(Eigen::MatrixXd& offline_pred_frame_objects,
                             ...
                             Eigen::MatrixXd& truth_frame_poses) {
  std::cout << "incremental_build_graph " << endl;
  ...
  for (int frame_index = 0; frame_index < total_frame_number; frame_index++) {
    std::cout << "reading image :" << frame_index << endl;
    ...
    std::cout << "online_detect_mode = " << online_detect_mode << endl;
    online_detect_mode = 1;
    // read or detect
    // cuboid　如果進行線上的ｃｕｂｅ檢測，只需要提供ｂｂｏｘ即可；
    if (online_detect_mode) {
      // start detect cuboid
      std::string imagename =
          base_folder + "raw_imgs/" + frame_index_c + "_rgb_raw.jpg";
      cv::Mat raw_rgb_img = cv::imread(imagename, 1);  // for 1 means color
      cout << "current image name " << imagename << std::endl;
      cout << "current image type " << raw_rgb_img.type() << endl;
      // edge detection
      ...
      if (detect_cuboid_obj.whether_sample_cam_roll_pitch)  // sample around
                                                            // first frame's pose
...
int main(int argc, char* argv[]) {
  ...
  // base_folder = "/media/shinan/DATA/ROSbag/Cubeslam/partdata/";
  // online_detect_mode = true;
  // save_results_to_txt = true;
  ...
  // return -1;
  if (!read_all_number_txt(truth_camera_pose, truth_frame_poses)) return -1;
